...
function initRegulatorTable() {
	regulatorTable = $('#mainTable').DataTable({
		// url
		"sAjaxSource" : "fault/queryTowerTileAlarmList", 
		"sScrollY":"370px",
		...
		*/
		...
		,{	
			 ...
			 "render": function(data, type, row, meta) {
				 var content = "";
				  content = '<button class="btn btn-xs blue" onclick="showEditModal(\''+row.id+'\') " data-toggle="modal" data-target="userModal_add"> 处理 </button>';
		         return content;
		      } 
		 }]
	});
...
function showEditModal(recordId){
	...
	$.ajax({
		url:"fault/setDealTowerTileAlarmById",
		type:"post",
		data:dataObj,
		dataType:"text",
		success:function(data) {
		...
		error:function(e) {
			stopPageLoading()
		   showSuccessOrErrorModal("设置杆塔倾斜报警已处理请求出错了","error"); 
		}
	});
}
